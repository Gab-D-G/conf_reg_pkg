...
def regress(scan_info, bold_files, brain_mask_files, confounds_files, conf_list, TR, lowpass, highpass, smoothing_filter, out_dir):
    import pandas as pd
    import numpy as np
    import nilearn.image

    def find_scans(scan_info, bold_files, brain_mask_files, confounds_files):
        ...
        for file in confounds_files:
            if scan_info in file:
                confounds_file=file
                break
        return bold_file, brain_mask_file, confounds_file

    bold_file, brain_mask_file, confounds_file=find_scans(scan_info, bold_files, brain_mask_files, confounds_files)
    ...
    smoothed=nilearn.image.smooth_img(bold_file, smoothing_filter)
    if len(confounds_list)>0:
        confounds_array=np.transpose(np.asarray(confounds_list))
        cleaned = nilearn.image.clean_img(smoothed, detrend=True, standardize=True, low_pass=lowpass, high_pass=highpass, confounds=confounds_array, t_r=TR, mask_img=brain_mask_file)
    else:
        cleaned = nilearn.image.clean_img(smoothed, detrend=True, standardize=True, low_pass=lowpass, high_pass=highpass, confounds=None, t_r=TR, mask_img=brain_mask_file)
...
regress_node = pe.Node(Function(input_names=['scan_info', 'bold_files', 'brain_mask_files', 'confounds_files', 'conf_list', 'TR', 'lowpass', 'highpass', 'smoothing_filter', 'out_dir'],
